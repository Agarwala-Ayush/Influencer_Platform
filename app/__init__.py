...
Arsonist = Flask(__name__)

current_directory = os.path.abspath(os.path.dirname(__file__))
parent_of_parent_directory = os.path.abspath(os.path.join(current_directory, os.pardir))
db_directory = os.path.join(parent_of_parent_directory, "instance", "arsonist.sqlite3")

Arsonist.config["SQLALCHEMY_DATABASE_URI"] = "sqlite:///" + db_directory
db = SQLAlchemy(Arsonist)
enc = Bcrypt(Arsonist)
api = Api(Arsonist)

def app_creator():
    Arsonist.config["SECRET_KEY"] = "@Emperor_Noctivagous"
    Arsonist.config["PERMANENT_SESSION_LIFETIME"] = timedelta(days=30)
    Arsonist.config["UPLOAD_FOLDER"] = "app/static/img/profile"
    ...
    Arsonist.register_blueprint(auth_bp)
    Arsonist.register_blueprint(dashboard_bp)
    ...
    Arsonist.run(debug=True, port=6969)
